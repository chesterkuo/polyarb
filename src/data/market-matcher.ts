...
interface TeamNames { lol: TeamDictionary; dota2: TeamDictionary; cs2: TeamDictionary; nba: TeamDictionary }
...
export class MarketMatcher {
  ...
  match(market: Market, liveMatches: LiveMatch[]): string | null {
    ...
    const sport = this.detectSport(q);
    if (!sport) return null;

    for (const match of liveMatches) {
      if (sport !== match.game && !(sport === 'nba' && match.game === undefined as any)) continue;
